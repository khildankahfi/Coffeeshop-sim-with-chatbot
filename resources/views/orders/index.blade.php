...
@push('styles')
<style>
.orders-stats { display: grid; grid-template-columns: repeat(4,1fr); gap: 16px; margin-bottom: 28px; }
...
.filter-btn { padding: 6px 16px; border-radius: 20px; font-size: 12px; cursor: pointer; border: 1px solid #d4b896; color: var(--brown-main); background: transparent; font-family: 'DM Sans', sans-serif; transition: all .2s; }
...
<div class="filter-bar">
    ...
    <a href="{{ route('admin.orders.index', ['status' => $s]) }}">
        <button class="filter-btn {{ request('status') === $s ? 'active' : '' }}">{{ ucfirst($s) }}</button>
    </a>
    @endforeach
</div>
    ...
    <table class="orders-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @php
                    $badgeClass = ['pending'=>'badge-pending','processing'=>'badge-processing','done'=>'badge-done','cancelled'=>'badge-cancelled'][$order->status] ?? 'badge-pending';
                    $icons = ['pending'=>'⏳','processing'=>'🔄','done'=>'✅','cancelled'=>'❌'];
                    @endphp
                    <span class="badge {{ $badgeClass }}">{{ $icons[$order->status] ?? '' }} {{ ucfirst($order->status) }}</span>
                </td>
                <td>
...
<div class="empty-state">
    <div class="emoji">📋</div>
    <p>Belum ada order masuk. Order dari pelanggan via chat Karen akan muncul di sini.</p>
</div>
@endif
@endsection